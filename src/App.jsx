...
import { NQA_SECTIONS, INIT_NQA_SYSTEMS, INIT_NQA_SCENARIOS, INIT_NQA_PLANS } from "./nqa/data.js";
import { NqaScreen, NqaTargetScreen, NqaScenarioScreen, NqaPlanScreen } from "./nqa/screens.jsx";
...
export default function App() {
  ...
  const api = {
    ...
    nqaPlans, addNqaPlan: (p) => setNqaPlans((x) => [...x, withCreate(p)]), updateNqaPlan: (id, patch) => setNqaPlans((x) => x.map((p) => (p.id === id ? { ...p, ...withUpdate(patch) } : p))), removeNqaPlan: (id) => setNqaPlans((x) => x.filter((p) => p.id !== id)),
    ...
    datasets, addDataset: (d) => setDatasets((x) => [...x, withCreate(d)]), updateDataset: (id, patch) => setDatasets((x) => x.map((d) => (d.id === id ? { ...d, ...withUpdate(patch) } : d))), removeDataset: (id) => setDatasets((x) => x.filter((d) => d.id !== id)),
