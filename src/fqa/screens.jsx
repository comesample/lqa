...
import {
  ...
  Smartphone, Sparkles, ChevronRight, ChevronDown,
} from "lucide-react";
...
export function FqaDashboardScreen({ nav }) {
  ...
  return (
    <div className="space-y-4">
      <PageToolbar desc="자동화 현황 · 스위트 건강도" />
      <div className="grid grid-cols-5 gap-3">
        ...
        <div className="col-span-9 space-y-3">
          ...
          <Card className="p-4 space-y-3">
            ...
            <div className="rounded-lg border border-slate-800 p-3 space-y-2.5">
              <div className="flex items-center justify-between"><span className="text-xs font-semibold text-slate-300">소스 저장소 (SCM)</span><Badge kind="pass">연결됨</Badge></div>
              <div className="grid grid-cols-3 gap-2">
                <Field label="제공자"><Select defaultValue="GitLab"><option>GitLab</option><option>GitHub</option><option>Bitbucket</option></Select></Field>
                <div className="col-span-2"><Field label="저장소"><Input defaultValue="gitlab.skt/tworld/web" /></Field></div>
              </div>
              <div className="grid grid-cols-3 gap-2">
                <Field label="기본 브랜치"><Input defaultValue="main" /></Field>
                <div className="col-span-2"><Field label="인증"><div className="rounded-lg border border-slate-800 bg-slate-900 px-2.5 py-2 text-xs text-slate-400">Personal Access Token · <span className="font-mono">••••••</span> (Secrets)</div></Field></div>
              </div>
              <div className="flex justify-end"><Btn icon={RefreshCw} onClick={() => flash("GitLab 저장소 연결 테스트 통과 · 브랜치 main 확인")}>연결 테스트</Btn></div>
            </div>
            <div className="rounded-lg border border-slate-800 p-3 space-y-2.5">
              <div className="flex items-center justify-between"><span className="text-xs font-semibold text-slate-300">CI/CD 연동</span><Badge kind="info">인바운드 웹훅</Badge></div>
              <Field label="수신 웹훅 URL (플랫폼 발급)"><div className="flex items-center gap-2"><div className="flex-1 truncate rounded-lg border border-slate-800 bg-slate-900 px-2.5 py-2 font-mono text-xs text-slate-300">https://xq.skt/api/hooks/tw-web-3f9a2c</div><Btn onClick={() => flash("웹훅 URL이 복사되었습니다")}>복사</Btn></div></Field>
              <div className="grid grid-cols-2 gap-2">
                <Field label="제공자"><Select defaultValue="GitLab CI"><option>GitLab CI</option><option>Jenkins</option><option>GitHub Actions</option></Select></Field>
              ...
              <div className="text-xs text-slate-500">이 URL을 GitLab 프로젝트 웹훅에 등록하면 커밋·PR·파이프라인 이벤트가 수신됩니다.</div>
            </div>
            <div className="rounded-lg border border-slate-800 p-3 space-y-2.5">
              ...
              <div className="grid grid-cols-2 gap-2">
                <Field label="배포 신호 (배포 기준)"><Select defaultValue="CD 배포 완료 웹훅"><option>릴리스 태그(v*)</option><option>CD 배포 완료 웹훅</option><option>이미지 태그 push</option></Select></Field>
              ...
              <div className="text-xs text-slate-500">배포 신호 발생 시 이 환경 대상으로 트리거가 실행되며, 버전은 결과 스냅샷에 기록됩니다.</div>
            </div>
          </Card>
